...
/* NOVLAN-BEGIN */
	a = [['_ppp_idletime', 3, 1440],['_ppp_redialperiod', 1, 86400],['_f_wan_mtu', 576, 1500],
		 ['_dhcp_lease', 1, 10080],['_pppoe_lei', 0, 30],['_pppoe_lef', 0, 10]];
/* NOVLAN-END */
/* VLAN-BEGIN */
	a = [['_ppp_idletime', 3, 1440],['_ppp_redialperiod', 1, 86400],['_f_wan_mtu', 576, 1500],
		['_pppoe_lei', 0, 60],['_pppoe_lef', 0, 10]];
/* VLAN-END */
	for (i = a.length - 1; i >= 0; --i) {
		v = a[i];
...
/* DNSCRYPT-END */
	...
	if (fom.f_lan_state.checked) fom.lan_state.value |= 1;
	if (fom.f_lan_desc.checked) fom.lan_state.value |= 2;
	if (fom.f_lan_invert.checked) fom.lan_state.value |= 4;
...
/* LINUX26-END */
	...
	{ title: 'Redial Period', indent: 2, name: 'ppp_redialperiod', type: 'text', maxlen: 5, size: 7, suffix: ' <i>(seconds)</i>',
	...
	{ title: 'LCP Echo Failure Threshold', indent: 2, name: 'pppoe_lef', type: 'text', maxlen: 5, size: 7, suffix: ' <i>(retries)</i>',value: nvram.pppoe_lef },
	{ title: 'Override MTU', multi: [
...
<div class='section-title'>Ethernet Monitor Status - Settings</div>
<div class='section'>
<script type='text/javascript'>
createFieldTable('', [
	{ title: 'Enable Ports Status', name: 'f_lan_state', type: 'checkbox', value: (nvram.lan_state & 1) },
	{ title: 'Speed Info', indent: 2, name: 'f_lan_desc', type: 'checkbox', value: (nvram.lan_state & 2) },
	{ title: 'Swap Ports Order', indent: 2, name: 'f_lan_invert', type: 'checkbox', value: (nvram.lan_state & 4) }
]);
</script>
</div>
...
//	if (wl_ifaces.length > 1)
	...
	f = [
		...
		{ title: 'SSID', name: 'wl'+u+'_ssid', type: 'text', maxlen: 32, size: 34, suffix: ' <i>(use only ASCII characters, avoid symbols!)</i>', value: eval('nvram.wl'+u+'_ssid') },
